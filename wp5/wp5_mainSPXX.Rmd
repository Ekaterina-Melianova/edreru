...
saveRDS(df_region_returns_college,file="df_reg_ret_coll.rds")
saveRDS(df_region_returns_university,file="df_reg_ret_univ.rds")
```
...
df_region_returns_univ18 <- df_region_returns_university %>% filter(year == 2018)
df_region_returns_coll18 <- df_region_returns_college %>% filter(year == 2018)

# Double universities dataframe
df_region_returns_univ18_full <- rbind(df_region_returns_univ18, df_region_returns_univ18)
df_region_returns_univ18_full$returns <- c(df_region_returns_univ18$social_returns, df_region_returns_univ18$private_returns)
df_region_returns_univ18_full$returns_type <- c(rep("Social", nrow(df_region_returns_univ18)), rep("Private", nrow(df_region_returns_univ18)))

# Double colleges dataframe
df_region_returns_coll18_full <- rbind(df_region_returns_coll18, df_region_returns_coll18)
df_region_returns_coll18_full$returns <- c(df_region_returns_coll18$social_returns, df_region_returns_coll18$private_returns)
df_region_returns_coll18_full$returns_type <- c(rep("Social", nrow(df_region_returns_coll18)), rep("Private", nrow(df_region_returns_coll18)))
...
# Plot the salaries by the areas
ggplot(df_region_returns_univ18_full, aes(x=reorder(region_name, returns), y=returns, color=returns_type)) +
  geom_point(size=3) +
...
# Plot the salaries by the areas
ggplot(df_region_returns_coll18_full, aes(x=reorder(region_name, returns), y=returns, color=returns_type)) +
  geom_point(size=3) +
  coord_flip() + theme_light() + 
  theme(plot.title = element_text(size = 22, hjust = 0.9), axis.text = element_text(size=12), legend.text = element_text(size = 14), legend.title=element_text(size=14)) +
  labs(x="Region", y = "Internal rate of return") + 
...
The average levels of social returns of college and university graduates in a region are highly correlated - regions with a high level of university returns tend to have higher rates of college returns. Nonetheless, there are some exceptions: for example, Primorskiy Kray has above average social returns of college graduates, but lacking behind in university returns. The situation in Kamchatskiy Kray is directly the opposite - the region is characterized by one of the highest levels of social university returns, but the college returns are a lot lower than in most of the other regions. You can see a distribution of returns by two levels of education on the scatterplot below.


```{r Plot 3. Scatterplot. Social Returns by regions, echo=FALSE, fig.height=8, fig.width=8}

# Preprocessing for plotting
df_temp.1 <- df_region_returns_univ18_full %>%
  filter(returns_type == "Social") %>%
  dplyr::select(region_name, social_returns) %>%
  rename(social_returns_university=social_returns)
df_temp.2 <- df_region_returns_coll18_full %>%
  filter(returns_type == "Social") %>%
  dplyr::select(region_name, social_returns) %>%
  rename(social_returns_college=social_returns)
df_region_returns_univ18_college <- left_join(df_temp.1, df_temp.2, by="region_name")


region_names <- df_region_returns_univ18_college$region_name
point_labels <- ifelse("Respublika Tatarstan" == region_names, "Respublika Tatarstan",
                       ifelse("Sakhalinskaya Oblast" == region_names, "Sakhalinskaya Oblast",
                              NA))

# Plot the scatterplot
plot1 <- ggplot(df_region_returns_univ18_college, aes(x=social_returns_university, y=social_returns_college)) +
  geom_point(size=4, color='gray') + theme_minimal() + theme(plot.title = element_text(size = 18), axis.text = element_text(size=14)) + geom_hline(yintercept=mean(df_region_returns_univ18_college$social_returns_college, na.rm=T), linetype="dashed", color = "red", size=1) + geom_vline(xintercept=mean(df_region_returns_univ18_college$social_returns_university, na.rm=T), linetype="dashed", color = "red", size=1) +
  labs(x="Social Returns, University", y = "Social Returns, College") + 
  ggtitle("Social Returns in Universities and Colleges by regions") + 
  geom_text(aes(label = point_labels), size = 3.5, nudge_x = -0.035, nudge_y = 0.001)
...
df_colleges_table <- df_colleges %>%
  dplyr::select(social_returns, private_returns, name, region_name, graduates_number) %>%
...
# Change columns for visualization
colnames(df_colleges_table) <- c("social returns", "private returns", "name", "region", "graduates number")
