...
plot1 <- ggplot(df_universities, aes(reorder(specialization_type, salary_2014/12), salary_2014/12)) +
  ...
  coord_flip() +
  ggtitle("Salary of Univ Graduates in 2014 \n by Specialization") + 
...
plot2 <- ggplot(df_colleges, aes(reorder(speciality_type, graduate_salary_2014/12), graduate_salary_2014/12)) +
  geom_boxplot(outlier.shape = NA, fill="lightcoral") + 
   coord_flip() +
  ggtitle("Salary of College Graduates in 2014 \n  by Specialization") +
...
# Foregone wage of Secondary only
wage_regions <- df_rosstat %>% filter(YEAR == 2014) %>% 
                    filter(edu_4 == "Secondary") %>%
                    group_by(H00_02) %>%
...
## Now add regional names in English

reg_wp3 <- openxlsx::read.xlsx("C:/Country/Russia/Data/SEASHELL/SEABYTE/edreru/wp3/RoREs_cleaned.xlsx")

df_region_returns_university <-
  left_join(df_region_returns_university,
             reg_wp3,
             by=c("region"="OKATO"))

df_region_returns_college <-
  left_join(df_region_returns_college,
             reg_wp3,
             by=c("region"="OKATO"))
...
df_region_returns_university  <- df_region_returns_university  %>% mutate(Dep_reg=ifelse(en_rgnames %in% depressed_regions,1,0))

df_region_returns_college <-
df_region_returns_college %>%
mutate(Dep_reg=ifelse(en_rgnames %in% depressed_regions,1,0))


# define a variable of difference between private and social 
df_region_returns_university$psgapu <- df_region_returns_university$private_returns-
  df_region_returns_university$social_returns

df_region_returns_college$psgapc <- 
df_region_returns_college$private_returns-
 df_region_returns_college$social_returns   

saveRDS(df_region_returns_college, "df_reg_ret_coll.rds")
saveRDS(df_region_returns_university,"df_reg_ret_univ.rds")
...
```{r Plot 1. Mean Social and Private Returns by regions, fig.height=14, fig.width=10, echo=FALSE}
# For color of depressed regions
setwd("C:/Country/Russia/Data/SEASHELL/SEABYTE/edreru/wp5")
...
# Arranging by Social returns only
df_region_returns_university_full1 <- df_region_returns_university_full[df_region_returns_university_full$returns_type == 'Social',]
df_region_returns_university_full1 <- df_region_returns_university_full1 %>% arrange(returns)

# Assinging levels to a factor with regions
df_region_returns_university_full$en_rgnames <-
  factor(df_region_returns_university_full$en_rgnames,
         levels = df_region_returns_university_full1$en_rgnames)

# Obtaining a vector with colors
df_region_returns_university_full <- df_region_returns_university_full %>% arrange(returns)
col_HE <- ifelse(levels(df_region_returns_university_full$en_rgnames) %in% priority_regions, "red", "black")

# Plot the salaries by the areas
ggplot(df_region_returns_university_full, aes(x=en_rgnames, 
                                              y=returns, color=returns_type)) +
  geom_point(size=3) +
  coord_flip() + theme_light() + 
  theme(plot.title = element_text(size = 22, hjust = 0.9), axis.text = element_text(size=12), legend.text = element_text(size = 14), legend.title=element_text(size=14), axis.text.y = element_text(color = col_HE, size = 14)) +
...
```{r Plot 2. Mean Social and Private Returns by regions, fig.height=14, fig.width=10, echo=FALSE}

setwd("C:/Country/Russia/Data/SEASHELL/SEABYTE/edreru/wp5")
# Plot the salaries by the areas
ggplot(df_region_returns_college_full, aes(x=reorder(en_rgnames, returns), y=returns, color=returns_type)) +
  geom_point(size=3) +
  coord_flip() + theme_light() + 
  theme(plot.title = element_blank(),
        axis.title.y = element_blank(),
        axis.text = element_text(size=12), legend.text = element_text(size=14),                      
        legend.title=element_text(size=14),legend.position=c(0.20,0.80)) +
    labs(y = "Internal rate of return") + 
   scale_color_manual(values=c("deepskyblue1", "green"), 
                    name="",labels=c("Social","Private"))
...
```{r Plot 51. Boxplots Social Returns by areas, echo=FALSE, fig.height=12, fig.width=25}
...
```{r Plot 4.1. Scatterplot. Social-Private gap with income gap HE, echo=FALSE, fig.height=8, fig.width=8}
setwd("C:/Country/Russia/Data/SEASHELL/SEABYTE/edreru/wp5")
...
# Plot the scatterplot
plot1 <- ggplot(df_region_returns_university, aes(x=reg_minckfd, y=diff_returns)) +
  geom_point(size=4, color='gray') + theme_minimal() + theme(plot.title = element_text(size = 18), axis.text = element_text(size=14)) + 
  labs(x="Income gap", y = "Social-Private gap") + 
  ggtitle("Social-Private gap and Income gap, Higher Education")
...
```{r Plot 4.2. Scatterplot. Social-Private gap with income gap VE, echo=FALSE, fig.height=8, fig.width=8}
...
# Plot the scatterplot
plot1 <- ggplot(df_region_returns_college, aes(x=reg_minckfd, y=diff_returns)) +
  geom_point(size=4, color='gray') + theme_minimal() + theme(plot.title = element_text(size = 18), axis.text = element_text(size=14)) + 
  labs(x="Income gap", y = "Social-Private gap") + 
  ggtitle("Social-Private gap and Income gap, Vocational Education")
...
#register_google('AIzaSyCgMUoTSCJ2fXSj3Au6XnGwrAyk1BnQbcQ', write = TRUE)
...
#register_google('AIzaSyBXXfPn9HfpPLzLQUyYjMVoUwvN7MRgNnk', write=TRUE)
map_spb <- get_googlemap(center="Saint-Petersburg", zoom=11, maptype='roadmap', language = 'ru', color='bw',
                        source='google', size = c(1000, 1000),
                        style = c(feature="all",element="labels",visibility="off")) 
...
```{r Linear regression with returns as DV Colleges, echo=FALSE}
...
#summary(lr_model_social)
#summary(lr_model_private)
