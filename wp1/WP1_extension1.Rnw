...
<<Packages, echo=T, cache=F, results=hide>>=
library(dplyr)
library(sqldf)
...
<<Data preprocessing, echo=T, cache=F, include=T, results=hide>>=
# wd
...
<<Regressions with depreciation of education, echo=T, cache=F, include=T, results=hide>>=
...
<<Plotting, echo=T, cache=F, include=T, results=hide>>=
