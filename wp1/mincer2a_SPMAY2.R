...
# sparkTable was removed from CRAN
# "Archived on 2020-04-02 as requested by the maintainer."
# I access an earlier installed version, using lib.loc command
# also needs "Cairo" and "Rglpk" to be installed
library(Cairo)
library(Rglpk)
library("sparkTable",lib.loc="C:/Users/wb164718/Documents/R/win-library/3.5")
...
g1 <- ggplot(temp_,aes(x=AGE,y=wage_c18,group=as.factor(edu_4),color=as.factor(edu_4))) +
  ...
  scale_linetype_manual(values = c('dotted','longdash', 'solid'), guide = 'none') +
  coord_cartesian(ylim=c(5000,40000))+
  scale_color_manual(values=c("darkred","darkgreen","#824CA7"),
  ...
  ggtitle('1998') +
  guides(shape = FALSE,
         colour = guide_legend(override.aes = list(color = c("darkred","darkgreen","#824CA7"),
...
ggplot(RoREs_edu, aes(YEAR, value, group = variable, color = variable,
  ...
  scale_color_manual(values = c("darkgray", "darkmagenta", "black")) +
