...
df6 <- data.frame(matrix(ncol = 8, nrow = 466))
colnames(df6) <- c("IC1","IC2","IC3","IC4","R1","R2","R3","R4") # Indirect cost and replace which Y values
...
df_universities3 <- cbind(df_universities2,df6)  # 8 values added
...
df_universities3$private_cost <- df_universities3$paidServices_mean / (df_universities3$graduates_number*4)
...
df_universities3$income_total_mean <- df_universities3 %>%
  ...
  rowMeans(na.rm = T, dims = 1)
# Calculate social cost per graduate
df_universities3$fiscal_cost <- df_universities3$income_total_mean / (df_universities3$graduates_number*4)
...
blix <- df_universities3 %>% 
  dplyr::select(SPUNID,private_cost,fiscal_cost,starts_with("X"),starts_with("Y"),IC1:IC4,R1:R4) %>% arrange(SPUNID)
glimpse(blix)
...
for (i in seq_along(1:2)){
 ...
 cp1_ <- af %>% dplyr::select(IC1:IC4)
 cp1 <- c(cp1_,ydf) %>% as.numeric()
 print(cp1)
}
